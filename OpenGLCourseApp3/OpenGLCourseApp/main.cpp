...
void CreateObjects()
{
	...
	GLfloat vertices[] = {
		//	x      y      z			u	  v(texture)			nx	  ny    nz(normals)
...
int main()
{
	...
	camera = Camera(glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), -90.0f, 0.0f, 0.1f, 0.1f);
...
void CreateObjects()
{
	...
	GLfloat vertices[] = {
		// x, y, z, u, v (u, v), normal x, normal y, normal z are the texture coordinates
...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		// Getting the time for deltaTime
		...
		uniformAmbientIntensity = shaderList[0].GetAmbientIntensityLocation();
		uniformDirection = shaderList[0].GetDirectionLocation();
		uniformDiffuseIntensity = shaderList[0].GetDirectionLocation();
